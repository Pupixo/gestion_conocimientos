...
@Component({
    selector: 'app-modals_preguntas_examen_curso.',
    templateUrl: './modals_preguntas_examen_curso.html',
    styleUrls: ['./modals_preguntas_examen_curso.css'],
    animations: [
...
export class ModalPreguntasExamenCursoComponent implements OnInit  {
    @Output() eventoHijo = new EventEmitter<void>();
    formulario_modal!: FormGroup;
    usuario_data: any={};
    data: any={};
    params_preguntas_examen_curso!: FormGroup;
    ...
    initForm() {
        this.params_preguntas_examen_curso = this.fb.group({
            id: [0],
            examen_curso: ['', Validators.required],
    ...
    InicializarDatos(){
       ...
       if (Object.keys(this.data).length === 1) {
            ...
            this.params_preguntas_examen_curso.patchValue({
                examen_curso: this.data.examen_curso,
    ...
    subirArchivo(event: any, tipo_file: string, form_control: string) {
        ...
        if  (tipo_file === 'img' && !this.isValidImage(file.type)) {
                ...
                // form_control
                this.params_preguntas_examen_curso.get(form_control)!.setValue('');
    ...
    verArchivo(controlName: string) {
        ...
        const fileUrl = this.params_preguntas_examen_curso.get(controlName)?.value ?? '';
    ...
    GuardarDataPreguntas() {
        ...
        const pregunta = this.params_preguntas_examen_curso.get('pregunta')?.value ?? '';
        if (!pregunta) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar el titulo para el examen" });
            return;
        }
    
        const opcion1 = this.params_preguntas_examen_curso.get('opcion1')?.value ?? '';
        if (!opcion1) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar el tiempo del examen",
                padding: '2em',
                 });
            return;
        }

        const opcion2 = this.params_preguntas_examen_curso.get('opcion2')?.value ?? '';
        if (!opcion2) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar el onjetivo del examen",
                padding: '2em',
                 });
            return;
        }

        const opcion3 = this.params_preguntas_examen_curso.get('opcion3')?.value ?? '';
        if (!opcion3) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar el onjetivo del examen",
                padding: '2em',
                 });
        ...
        const opcion4 = this.params_preguntas_examen_curso.get('opcion4')?.value ?? '';
        if (!opcion4) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar el onjetivo del examen",
                padding: '2em',
                 });
        ...
        const opcion5 = this.params_preguntas_examen_curso.get('opcion5')?.value ?? '';
        if (!opcion5) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar el onjetivo del examen",
                padding: '2em',
                 });
        ...
        if (this.params_preguntas_examen_curso.get('opcion1_correcta')?.value) {
        opciones_correctas.push(0);
        }
        if (this.params_preguntas_examen_curso.get('opcion2_correcta')?.value) {
        opciones_correctas.push(1);
        }
        if (this.params_preguntas_examen_curso.get('opcion3_correcta')?.value) {
        opciones_correctas.push(2);
        }
        if (this.params_preguntas_examen_curso.get('opcion4_correcta')?.value) {
        opciones_correctas.push(3);
        }
        if (this.params_preguntas_examen_curso.get('opcion5_correcta')?.value) {
        opciones_correctas.push(4);
        }
        console.log("🚀 ~ ModalPreguntasExamenCursoComponent ~ GuardarDataPreguntas ~ opciones_correctas.length:", opciones_correctas.length)

        if (opciones_correctas.length==0) {
            Swal.fire({ icon: "error", title: "Oops...", text: "Debe ingresar al menos una alternativa correcta",
                padding: '2em',
                 });
            return;
        }
        ...
        if(id == 0){
            var imagen_pregunta = this.params_preguntas_examen_curso.get('imagen_pregunta')?.value ?? '';
            if (!imagen_pregunta) {
            ...
            formData.append('examen_curso', this.data.examen_curso);
            ...
            this.gestioncursoservice.registrarPreguntasExamenCurso(formData).subscribe(
                respuesta => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Se registro',
                    ...
                    this.notificarPadre("cerrar_modal_preguntas_examen_curso");
                },
                error => {
                    Swal.fire({
                        icon: "error",
            ...
            this.gestioncursoservice.EditarPreguntasExamenCurso(formData,id).subscribe(
                respuesta => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Se Actualizo',
